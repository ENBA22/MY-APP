...
function home () {
    ...
    useEffect(() => {
      axios
        .get("http://localhost:3001/get/"+username) // Include username in the request
        .then((result) => setTodos(result.data))
        .catch((err) => console.log(err));
    }, [username]);

    const handleedit = (id, username) => { 
      axios.put(`http://localhost:3001/update/${id}`, { username: username }) 
      .then(result => {
          location.reload();
      })
      .catch(err => console.log(err));
    ...
    const handletodo = (id, username) => { 
      axios.put(`http://localhost:3001/todoo/${id}`, { username: username }) 
      .then(result => {
          location.reload();
      })
      .catch(err => console.log(err));
    ...
    const handledone =(id, username)=>{
      axios.put(`http://localhost:3001/done/${id}`, { username: username })
      .then(result => {
        location.reload()
      })
      .catch(err => console.log(err))
    ...
    const handledel =(id, username)=>{
      axios.put(`http://localhost:3001/del/${id}`, { username: username })
      .then(result => {
        location.reload()
      })
      .catch(err => console.log(err))
